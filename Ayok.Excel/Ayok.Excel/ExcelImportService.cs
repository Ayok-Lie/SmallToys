...
namespace Ayok.Excel
{
    ...
    {
        public List<T> ImportData<T>(Stream stream, bool hasHeader = true) where T : new()
        {
            using ExcelPackage excelPackage = new ExcelPackage(stream);
            ExcelWorksheet excelWorksheet = excelPackage.Workbook.Worksheets.FirstOrDefault() ?? throw new InvalidOperationException("Excel文件中未找到有效工作表");
            ...
            if (hasHeader)
            {
                ...
                if (dimension != null && dimension.Rows >= 1)
                {
                    for (int i = 1; i <= excelWorksheet.Dimension.End.Column; i++)
                    {
                        ...
                        PropertyInfo propertyInfo = properties.FirstOrDefault((PropertyInfo p) => p.Name.Equals(headerName, StringComparison.OrdinalIgnoreCase) || (p.GetCustomAttribute<ColumnAttribute>()?.Name ?? "") == headerName);
                        if (propertyInfo != null)
                        {
                            dictionary[i] = propertyInfo;
                            continue;
                        }
                        throw new InvalidOperationException("未找到与列名'" + headerName + "'对应的实体属性，请检查映射配置");
                    }
                    goto IL_0138;
                }
            }
            for (int num2 = 1; num2 <= Math.Min(properties.Length, excelWorksheet.Dimension.End.Column); num2++)
            {
            ...
            for (int num3 = num; num3 <= excelWorksheet.Dimension.End.Row; num3++)
            {
                ...
                foreach (KeyValuePair<int, PropertyInfo> item in dictionary)
                {
                    ...
                    if (excelRange.Value != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException($"第{num3}行第{key}列数据转换失败({value.Name}): {ex.Message}");
                        }
                    }
                }
                if (flag)
        ...
        private static object ConvertValue(object cellValue, Type targetType)
        {
            ...
            if (type.IsEnum)
            {
                try
                {
                    return (cellValue is string value) ? Enum.Parse(type, value, ignoreCase: true) : Enum.ToObject(type, cellValue);
                }
                catch (ArgumentException)
                {
                    throw new InvalidCastException($"值 '{cellValue}' 无法转换为枚举类型 {type.Name}");
                }
            }
            if (type == typeof(bool))
            {
